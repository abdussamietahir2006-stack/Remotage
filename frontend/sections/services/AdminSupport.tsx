...
const defaultBullets = [
  "Completely handle your email inbox",
...
export default function AdminSupport({ content = {}, images = {} }: Props) {
  const title   = content.service1Title  || "Administrative Support";
  const desc    = content.service1Desc   || "Let us take the administrative burden off your plate. Our dedicated virtual assistants manage the day-to-day operations so you can focus on what truly matters — growing your business.";
  const bullets = [
    content.service1Bullet1 || defaultBullets[0],
    content.service1Bullet2 || defaultBullets[1],
    content.service1Bullet3 || defaultBullets[2],
    content.service1Bullet4 || defaultBullets[3],
  ];
  const image = images.service1Image || "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?q=80&w=1400&auto=format&fit=crop";
      ...
      <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-16 items-center">
        <motion.div initial={{ opacity: 0, x: -80 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }} viewport={{ once: true }} className="relative group rounded-3xl overflow-hidden h-[480px]">
          ...
          <div className="absolute bottom-6 left-6">
            <span className="px-4 py-1.5 rounded-full bg-[#D4AF37]/10 border border-[#D4AF37]/30 text-[#D4AF37] text-sm tracking-widest uppercase">01</span>
          </div>
        </motion.div>

        <motion.div initial={{ opacity: 0, x: 80 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }} viewport={{ once: true }}>
          <span className="text-[#D4AF37] text-sm tracking-widest uppercase">Service One</span>
