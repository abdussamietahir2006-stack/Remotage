...
const defaultBullets = [
  "Enter and organize financial data",
  "Create invoices and track payments",
  "Record and categorize expenses",
  "Create financial summaries and reports",
];
...
export default function FinanceBookkeeping({ content = {}, images = {} }: Props) {
  const title   = content.service4Title  || "Finance & Bookkeeping";
  const desc    = content.service4Desc   || "Keep your finances organized and accurate without the stress. Our bookkeeping specialists handle the numbers so you always have a clear picture of your business health.";
  const bullets = [
    content.service4Bullet1 || defaultBullets[0],
    content.service4Bullet2 || defaultBullets[1],
    content.service4Bullet3 || defaultBullets[2],
    content.service4Bullet4 || defaultBullets[3],
  ];
  const image = images.service4Image || "https://images.unsplash.com/photo-1554224155-6726b3ff858f?q=80&w=1400&auto=format&fit=crop";
    ...
    <section className="py-28 px-6 bg-[#0D0D0D] overflow-hidden">
      <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-16 items-center">
        <motion.div initial={{ opacity: 0, x: -80 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }} viewport={{ once: true }}>
          <span className="text-[#D4AF37] text-sm tracking-widest uppercase">Service Four</span>
          <h2 className="text-4xl md:text-5xl font-bold text-white mt-3 mb-6 leading-tight">
            Finance &<br />
            <span className="bg-gradient-to-r from-[#D4AF37] via-[#F5E6A3] to-[#D4AF37] bg-clip-text text-transparent">Bookkeeping</span>
          </h2>
        ...
        <motion.div initial={{ opacity: 0, x: 80 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }} viewport={{ once: true }} className="relative group rounded-3xl overflow-hidden h-[480px]">
          ...
          <div className="absolute bottom-6 right-6">
            <span className="px-4 py-1.5 rounded-full bg-[#D4AF37]/10 border border-[#D4AF37]/30 text-[#D4AF37] text-sm tracking-widest uppercase">04</span>
          </div>
        </motion.div>
      </div>
    </section>
